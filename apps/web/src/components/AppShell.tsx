    ...
    <div className="flex min-h-screen bg-surface font-body text-on-surface">
      ...
      <aside className="fixed left-0 top-0 hidden h-screen w-64 flex-col border-r border-outline-variant/10 bg-surface-container p-6 space-y-8 z-50 md:flex">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                className={cn(
                  "flex w-full items-center gap-3 rounded-l-xl px-4 py-3 font-label text-[10px] font-semibold uppercase tracking-widest transition-all",
                  isActive
        ...
        <div className="pt-6 border-t border-outline-variant/10">
          <button
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 font-label text-sm font-medium text-on-primary shadow-lg transition-all hover:opacity-90 active:scale-95"
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                type="button"
              >
                <RefreshCw size={20} className={cn(saving && "animate-spin")} />
              </button>
      ...
      {/* Global FAB */}
      <button 
        className="fixed bottom-8 right-8 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-on-primary shadow-2xl transition-transform hover:scale-110 active:scale-95 group"
        onClick={onCreateNote}
      >
        <Plus size={24} />
